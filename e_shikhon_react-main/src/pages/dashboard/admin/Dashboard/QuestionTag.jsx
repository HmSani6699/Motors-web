import React, { useEffect, useState } from "react";
import file_edit from "../../../../assets/svg/file-document-edit-outline_green.svg";
import delete_red from "../../../../assets/svg/delete-sweep-outline_red.svg";
import InputField from "../../../../components/inputField/InputField";
import { del, get, post, put } from "../../../../api/axious";
import toast from "react-hot-toast";
import Loading from "../../../../components/sheared/Loading";
import leftArrowIcon from "../../../../assets/svg/arrow-sm-left.svg";
import Swal from "sweetalert2";
import { URL } from "../../../../constants/Url";

const QuestionTag = () => {
  const [loading, setLoading] = useState(false);
  const [update, setUpdate] = useState(false);
  const [create, setCreate] = useState(false);
  const [roleData, setRoleData] = useState();
  const [id, setId] = useState("");
  const [roleInput, setRoleInput] = useState("");
  const [questionTag, setQuestionTag] = useState("");

  useEffect(() => {
    fetchTag();
  }, []);

  const fetchTag = async () => {
    try {
      const response = await get(URL.QUESTION_TAG.GET);
      setRoleData(response?.data);
    } catch (error) {
      console.log("Error creating app:", error);
    }
  };

  const handleCreatePage = () => {
    setCreate(true);
  };

  const handleCreateCategory = async (event) => {
    event.preventDefault();
    setCreate(false);

    const formData = {
      tag: roleInput,
    };
    setLoading(true);
    try {
      const res = await post(URL.QUESTION_TAG.POST, formData);
      if (res.success) {
        fetchTag();
        setLoading(false);
        setRoleInput("");
        toast.success("Create successfully!");
      }
    } catch (error) {
      setLoading(false);
      // toast.error('failed to Post')
      console.log("Failed to post/", error?.response);
    } finally {
      setLoading(false);
    }
  };

  const handleSetUpdate = async (data) => {
    setUpdate(true);
    setId(data?.id);
    setQuestionTag(data?.tag);
  };

  const handleTagUpdate = async () => {
    setLoading(true);
    const payload = {
      tag: questionTag,
    };
    try {
      const res = await put(`${URL.QUESTION_TAG.PUT}/${id}`, payload);
      if (res.success) {
        setQuestionTag("");
        setId("");
        setUpdate(false);
        fetchTag();
        setLoading(false);
        // toast.success('Update role successfully!')
      }
    } catch (error) {
      setLoading(false);
      toast.error("faild to Update");
      console.log("Failed to post/", error?.response);
    } finally {
      setLoading(false);
    }
  };
  // handle delete tag
  const handleDeleteTag = async (id) => {
    setLoading(true);

    Swal.fire({
      title: "Are you sure?",
      text: "You won't be able to revert this!",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      confirmButtonText: "Yes, delete it!",
    }).then((result) => {
      if (result.isConfirmed) {
        try {
          const response = del(`${URL.QUESTION_TAG.DET}/${id}`);
          if (response) {
            fetchTag();
            Swal.fire({
              title: "Deleted!",
              text: "Your file has been deleted.",
              icon: "success",
              showConfirmButton: false,
              timer: 2000,
            });
            fetchTag();
          }
        } catch (error) {
          setLoading(false);
          toast.error("Failed to delete");
          console.log("Failed to delete/", error?.response);
        }
        fetchAllQuiz();
      }
    });
    setLoading(false);
  };
  return (
    <>
      <div className="px-[13px] w-full lg:w-[1100px] mb-10 mx-auto">
        <div className="bg-white rounded-[10px]">
          <div>
            <h2 className="text-[28px] font-[600] text_black_gray leading-[40px] p-2.5">
              Manage Question Tag
            </h2>
          </div>
        </div>
        {/*  */}
        <div className="p-1 mt-10 rounded-[10px] border-2 bg-white">
          <div className="w-full overflow-x-auto bg-white py-5 px-2.5 rounded-[10px]">
            <div className="border-b px-2.5 flex flex-wrap flex-row justify-between pb-2.5 mb-5">
              <div className="flex items-center gap-2">
                {create && (
                  <button onClick={() => setCreate(!create)} className="">
                    <img className="w-[25px] " src={leftArrowIcon} alt="" />
                  </button>
                )}
                {update && (
                  <button onClick={() => setUpdate(!update)} className="">
                    <img className="w-[25px] " src={leftArrowIcon} alt="" />
                  </button>
                )}
                <h1 className="text-[28px] font-[600] text_black leading-[36px]">
                  {!create && !update && "Question Tag List "}
                  {create && " Create New Question Tag "}
                  {update && " Update Question Tag "}
                </h1>
              </div>
              <div className="flex flex-row gap-3">
                {!create && (
                  <button
                    onClick={handleCreatePage}
                    className="p-2.5 text-white primary_green rounded-[5px] text-[14px] font-[400] leading-[18px]"
                  >
                    Add New Question Tag
                  </button>
                )}
              </div>
            </div>

            {/* form data */}

            {create && (
              <div>
                <div className="w-[650px] mx-auto">
                  <InputField
                    value={roleInput}
                    setValue={setRoleInput}
                    title="Tag Name"
                  />
                </div>
                <button
                  onClick={handleCreateCategory}
                  className="mt-5 px-3 py-2 rounded-[8px] mx-auto text-[16px] font-[500] leading-[24px] text-white primary_green flex flex-row items-center gap-2"
                >
                  {loading ? <Loading /> : "Submit"}
                </button>
              </div>
            )}

            {/* update role */}

            {update && (
              <div>
                <div className="w-[650px] mx-auto">
                  <InputField
                    defValue={questionTag}
                    setValue={setQuestionTag}
                    title="Update Tag Name"
                  />
                </div>
                <button
                  onClick={handleTagUpdate}
                  className="mt-5 px-3 py-2 rounded-[8px] mx-auto text-[16px] font-[500] leading-[24px] text-white primary_green flex flex-row items-center gap-2"
                >
                  Update
                </button>
              </div>
            )}

            {/* list */}

            {!create && !update ? (
              <table
                className="w-[650px] mx-auto  text-left border-collapse w-overflow-x-auto table-auto "
                cellSpacing="0"
              >
                <tbody>
                  <tr className="bg-[#F1F2F3] ">
                    <th
                      scope="col"
                      className="h-10 ps-2.5 text-[#2E3138] text-[16px] font-[500]"
                    >
                      No
                    </th>
                    <th
                      scope="col"
                      className="h-10 px-0 text-[#2E3138] text-[16px] font-[500] "
                    >
                      Tag Name
                    </th>
                    <th
                      scope="col"
                      className="h-10 px-0 text-[#2E3138] text-[16px] font-[500]"
                    >
                      Action
                    </th>
                  </tr>

                  {roleData?.map((data, i) => (
                    <tr key={i} className="border-b">
                      <td className="h-10 ps-2.5 py-2">
                        <h1 className="text_black_gray text-[16px] font-[500]">
                          {i + 1}
                        </h1>
                      </td>
                      <td className="h-10 py-2">
                        <div>
                          <h1 className="text_black_gray text-[16px] font-[500] leading-[22px]">
                            {data?.tag}
                          </h1>
                        </div>
                      </td>
                      <td className="h-10 py-2">
                        <div className="flex flex-row gap-2.5">
                          <button onClick={() => handleSetUpdate(data)}>
                            <img src={file_edit} alt="" />
                          </button>

                          <button onClick={() => handleDeleteTag(data?.id)}>
                            <img src={delete_red} alt="" />
                          </button>
                        </div>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : null}

            {/* -- */}
          </div>
        </div>
      </div>
    </>
  );
};

export default QuestionTag;
