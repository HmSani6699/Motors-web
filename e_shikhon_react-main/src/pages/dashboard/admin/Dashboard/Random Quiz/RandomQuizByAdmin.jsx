...
const RandomQuizByAdmin = () => {
  ...
  const fetchAllQuiz = async () => {
    try {
      ...
      const response = await get("/api/random_quiz/all_with_tags");
  ...
  const handleCreateQuiz = async () => {
    ...
    setLoading(true);
    try {
      const res = await post("/api/random_quiz/add", payload);
      console.log(res);
      if (res?.success) {
        resetForm();
        fetchAllQuiz();
  ...
  const handleQuizUpdateSubmit = async () => {
    ...
    try {
      const res = await put(
        `/api/random_quiz/update/${getOldQuizId}`,
        fromData
      );
      if (res.success) {
        fetchAllQuiz();
  ...
  const handleDeleteQuiz = async (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = del(`/api/random_quiz/delete/${id}`);
          if (response) {
            fetchAllQuiz();
            Swal.fire({
              title: "Deleted!",
...
export default RandomQuizByAdmin;
