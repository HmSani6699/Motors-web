...
const InstructorDashboard = () => {
    ...
    const [instructorProgressCount, setInsTructorProgressCount] = useState([]);

    const fetchInstructorProgress = async (id) => {
        setLoading(true);
        try {
            const res = await get(`api/auth/allCountsForInstructor/${id}`);
            console.log(res);
            setInsTructorProgressCount(res?.data);
            setLoading(false);
        } catch (error) {
            setLoading(false);
        } finally {
    ...
    useEffect(() => {
        if (user?.id) {
            fetchInstructorProgress(user?.id);
        }
    }, [user?.id]);

    // console.log(instructorProgressCount)
    return (
        <>
                ...
                <div className='grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-10'>
                    <div className='p-4 bg-[#FFE2E5] rounded-[16px]'>
                        <div className='flex flex-row lg:flex-col  gap-4'>
                            ...
                            <h1 className='text-[32px] font-[700] text-[#151D48]  leading-[30px] mt-1 md:mt-0'>{instructorProgressCount?.courses ? instructorProgressCount?.courses : 0}</h1>
                        </div>
                    ...
                    <div className=' p-5 bg-[#F3E8FF] rounded-[16px]'>
                        <div className='flex flex-row lg:flex-col gap-4'>
                            ...
                            <h1 className='text-[32px] font-[700] text-[#151D48]  leading-[30px] mt-1 md:mt-0'>{instructorProgressCount?.students ? instructorProgressCount?.students : 0}+</h1>
                        </div>
                        <h1 className='mt-5 lg:mt-0 text-[18px] font-[500] text-[#425166] leading-[28px]'>ছাত্রছাত্রী সংখ্যা</h1>
                    </div>
                    <div className=' p-5 bg-[#FFF4DE] rounded-[16px]'>
                        <div className='flex flex-row lg:flex-col gap-4'>
                            ...
                            <h1 className='text-[32px] font-[700] text-[#151D48]  leading-[30px] mt-1 md:mt-0'>{instructorProgressCount?.ratting ? instructorProgressCount?.ratting : 0}</h1>
                        </div>
